...
Since the Inverse-Probability-Weighting (IPW) approach, implemented in `coxrt`, relies on "blowing up" the sampled lifetimes $T_i$ in order to obtain the pseudo-population, which should recreate the same probability law as in the original population, `coxph.RT` is meant to fail in case there is an unobservable right tail of the distribution. Hence the function `coxph.RT.a0` aims to adjust for this violation of positivity by using the structural assumption of the Cox model in the population and the information on $a_0=P(T>r_*\mid Z=0)$.
